...
export const searchBookmarked = async () => {
  try {
    ...
    const likedPosts = posts.filter((post) =>
      post.usersIds.some((user) => user === currentAccount.$id)
    );
    return likedPosts;
  } catch (error) {
    throw new Error(error);
...
export const toggleLike = async (post) => {
  try {
    ...
    const isLiked = post.users?.some((user) => user === currentAccount.$id);

    if (isLiked) {
      post.users = post.users.filter((user) => user !== currentAccount.$id);
    } else {
      post.users.push(currentAccount.$id);
    }

    await databases.updateDocument(databaseId, videoCollectionId, post.id, {
      usersIds: post.users,
    });
